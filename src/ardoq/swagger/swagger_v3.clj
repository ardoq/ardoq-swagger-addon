...
(defn transform-parameter-objects [data spec parent-key]
  (let [current-key (str parent-key "/parameters")]
    (reduce
      (fn [data parameter-object-spec]
        (if (map-entry? parameter-object-spec)
          (transform-parameter-object (key parameter-object-spec) current-key data (val parameter-object-spec))
          (transform-parameter-object (:name parameter-object-spec) current-key data parameter-object-spec)))
      data
      (:parameters spec))))
...
(defn transform-path-object [data [path-object-key path-object-spec]]
  (let [key (str "#/paths/" (name path-object-key))
        data (-> data
               (update-in [:swagger-object key] assoc :name (name path-object-key))
...
(defn transform-components-object [data spec]
  (let [components-spec (:components spec)]
    (-> data
      ...
      (transform-parameter-objects components-spec "#/components")
      )))
...
(defn sync-component [client ardoq-data [spec-key spec-data-item]]
...
(defn sync-components [client ardoq-data spec-data]
  (doall (map (partial sync-component client ardoq-data) (:swagger-object spec-data))))
...
(defn import-swagger3 [client spec wsname]
  (let [ardoq-data (or
    ...
    (prn "synced")
    (clojure.pprint/pprint ardoq-sync-data)
